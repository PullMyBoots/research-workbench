...
Human ↔ Main Agent
             ...
             ├── 为适合持续单路线迭代的问题建立独立单 Agent 工作台
             └── 为适合多路线并行搜索的问题搭建探索 Team
                           ↓
                    Main 审阅与整合
                           ↓
                        最终成果
...
3. 根据任务特点选择 Main 直接工作、独立单 Agent 工作台、专项 Skill 或探索 Team。
4. 主动完成已授权范围内的研究与工程工作。
5. 审阅各类证据和 Team 进展，识别事实、推断、风险与取舍。
6. 将直接工作和外部工具的结果整合为可检查的最终成果。

Skill 是按任务调用的专业操作能力；独立单 Agent 工作台服务于需要局部长期记忆的持续单路线迭代；探索 Team 是面向高不确定性、多路线问题的并行搜索工具。Knowledge、Memory 和 Evaluation 是支持研究连续性与证据管理的基础设施，详细操作协议由对应 Skill 承担。
...
1. Main Memory
   - `.DiscoveryProgram/memory/main.md` 是新会话的入口简报，只保存目标与背景、元认知、当前进展与必要文件索引。
...
3. 单 Agent 课题记忆与知识库
   - 每个 `subprojects-single/<project-id>/` 拥有独立的 `.ResearchProject/`。
   - 它保存该课题的目标、元认知、进展 Logs 和外部知识，由所属 Project Agent 按本地 `AGENTS.md` 工作。
   - Main Agent 可以审阅和整合其成果，但不把该目录当作 Main 直接工作区或 Team Route。
...
2. 需要详细证据时，Main Agent 直接读取单 Agent 课题的 `.ResearchProject/`；自由知识库和团队课题知识库使用下述浏览 Skill。
...
- `$create-single-agent-project`：初始化一个由独立 Project Agent 持续迭代的课题，只写入 Human 提供的初始目标和空白项目记忆结构。
...
1. Main Agent 直接推进
   ...
   - Main Agent 在 `subprojects-main/<id>/` 创建空白文件夹并按实际需要组织内容；该目录不拥有独立模板、局部记忆或生命周期。

2. 独立单 Agent 迭代
   - 适用于一条主要研究路线需要跨会话持续实践、观察、经验提取和元认知更新，但不需要多路线 Team 的课题。
   - Human/Main 使用 `$create-single-agent-project` 在 `subprojects-single/<id>/` 初始化工作台，随后由一个 Project Agent 按本地规则持续推进。
